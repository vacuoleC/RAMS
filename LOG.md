...
- 改动文件：
  - 新建 `exp/model_enhancement/e_strat_trend/`（探索标记）：`corr_precheck.py`（相关性快速验证）、`run_e.py`（模型级对照）、`results.md`（结果）、`results.json`（统计量，无原始数据行）、`rethinking.md`、`trydoing.jsonl`、`corr_precheck.json`
- 执行命令与结果：
  - 相关性预检（本地 CPU，全数据 12607 时刻）：corr(Δconc, 分层趋势特征) 全部 |r|≤0.12（绝大多数 <0.08），Bootstrap95CI(3 seed) 几乎全含 0；按 trend_dT 正负分组的 Δconc 均值差 |Δmean|/std(Δconc)=0.016
  - H100 全量：`cd /data/RAMS/proj && nohup python3 exp/model_enhancement/e_strat_trend/run_e.py --seeds 3 --epochs 30 --device cuda` → 64.3 分钟，17 窗口 × 2 变体 × 3 seed
- 结果（3-seed 均值，17 窗口，conc 单位，abs_delta 协议）：
  - base（现输入）：CRPS 0.883（+20.8% vs 持久化 1.115）、RMSE 1.728、覆盖 0.804（std 0.078）
  - trend（+分层趋势 3 维）：CRPS 0.881（Δ=−0.2%）、RMSE 1.711（Δ=−1.0%）、覆盖 0.792（std 0.096，覆盖更差）
  - **结论：加分层趋势在模型级几乎无影响（Δ≤1%，噪声级），区间覆盖反而退化。相关性预检 + 模型级确认双证：分层趋势对增量无预测力。**
- 与 M5 对话：直接（水温→藻类，M5 无 MCI 边）+ 间接（分层趋势→Δ，本 E）两条物理链在数据上都不成立；增量信号主要由藻类自回归携带。探垂向混合需多站/亚网格数据或物理模型耦合。
- 失误：无（本地无 pandas 的 python3.14 默认解释器 → 改用 D:/enviranment/Python313）
- 冒烟：通过（本地 CPU 1 窗口 2 epoch + H100 全量）
- 交付物：`exp/model_enhancement/e_strat_trend/` 全套（脚本/结果/思考/日志）
- 状态：✅ 完成
- 下一步：不采纳分层趋势特征；若再探垂向混合物理链，需多站/物理耦合而非本站统计特征
