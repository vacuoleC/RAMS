...
### 2.1 研究区与数据
研究对象为华南某分层水库（站点已匿名化），监测期为 2021 年至 2025 年（约 4.5 年）。共获取 258,542 条藻类浓度观测，覆盖 20 个水深分层（0.5—10 m，间隔 0.5 m），以及 232,067 条气象观测（10 分钟采样间隔）。关键变量包括：水温、透光度、藻类浓度（绿藻/蓝藻/硅藻/隐藻）、总浓度，以及六项气象驱动因子（风速、温度、湿度、气压、降雨）。
