...
- **A 方向探索已完成（2026-08-10）**：增量目标 × 完整多任务叠加实证（3-seed 17 窗口 H100，见 `exp/model_enhancement/a_mt_increment/`）——**多任务不叠加点精度（轻微稀释），但把区间校准从欠覆盖拉回理想（覆盖 0.672→0.806）；增量×多任务的叠加是"校准"而非"精度"**。多任务 arm 复现 B7 abs_delta，持久化与 B7 逐位一致
- **J 方向探索已完成（2026-08-10）**：中长程慢变量特征（conc/air_temp 过去 30/60/90 天均值/斜率/方差，3-seed 17 窗口 H100，见 `exp/model_enhancement/j_slow_vars/`）——**假设部分证伪**：慢变量只帮覆盖窗口 std（0.0875→0.0818，-6.5%，H 漂移窗口 4/5 改善），但方向 AUC 显著变差（0.581→0.564，p=0.006）+ 点精度显著变差（RMSE +2.25%，p=0.023），**不值得整体加入**；慢变量与 24h Δ 相关全部 ≤0.037（季节状态对 Δ 方向/幅度几乎无预测力，与 E/D 负结果一致）——**方向判别弱不是缺季节状态造成**（信号在短程自回归，AUC 0.59 是数据上限），校准稳定性线索保留（把年际状态喂给不确定性估计作独立校准层，如 seas_reg 用慢变量替代月因子）
- **K 方向探索已完成（2026-08-10）**：两阶段训练解"精度 vs 校准"矛盾（3-seed 17 窗口 H100，见 `exp/model_enhancement/k_two_stage/`）——**ts_freeze（Stage1 单任务训 M1 20ep → Stage2 冻结 backbone 只训多头 10ep）是 4 arm 最优点精度（CRPS 0.808/RMSE 1.583，比单任务还低 5.7%，16:1 胜 p<0.001），覆盖率显著抬到 0.704（+3.2pp）但未追平多任务 0.806/80%——部分解开精度-校准矛盾**。冻结 backbone 是保住 Stage1 精度的必要条件（ts_full 解冻后精度回落 ≈单任务 p=0.927、校准仅 0.742）
- **E 方向探索已完成（2026-08-10）**：分层状态趋势喂增量（3-seed 17 窗口 H100，见 `exp/model_enhancement/e_strat_trend/`）——**分层趋势（delta_T/thermo_grad 3 天变化）对增量无预测力（负结果）**：corr(Δconc, 分层趋势)≈0（|r|≤0.12）+ 模型级确认（CRPS Δ=−0.2%、RMSE Δ=−1.0% 噪声级，覆盖反而退化 0.804→0.792）。与 M5 水温无因果中介互相印证：**"水温→藻类"直接路径（M5）+ "分层趋势→Δ"间接路径（E）两条物理链在数据上都不成立**，增量信号主要由藻类自回归携带；探垂向混合需多站/物理耦合
- **mdl-m4-warning 已完成（2026-08-10）**：M4 藻华预警召回/提前量/误报评估（3-seed × 17 窗口 H100，见 `docs/mdl_m4_warning_results.md`）——**θ=0.5 召回 2/5（评估期内 N 事件），命中事件平均提前 14.5 天（13-16 天），误报 6 段/84 天（10.6%）**；基线带 p75 召回 5/5 但只提前 4 天 + 14 误报段 → **模型用一半误报换 3.6 倍提前量**；12 事件对齐中 5 个出样本可评估（N8/N9 命中），7 个在滚动协议训练段内；误报集中在 2023-11/12 与 2024-10/11 秋冬长尾低幅抬升
- **下一步**：M1 精度优先 + 免费 M2/M4 + 校准优于单任务 → 生产可采纳 K 方向 ts_freeze（20+10，冻结 backbone）；覆盖达标 80% 仍保留多任务增量（w=1/3/2）或 ts_freeze 加事后共形/更长 Stage2；若要再压点精度走数据量/评估协议方向或探多任务权重（降 w_m2/w_m4）。M4 预警改进：标签改 3h 网格恢复 12 事件口径 / 加垂直联动确认门控压误报 / θ=0.5-0.6 为推荐工作点
